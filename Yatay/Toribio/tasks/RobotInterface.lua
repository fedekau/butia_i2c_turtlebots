...
local robotic_kit_es = 'robotic-kit-es.xml'
local robotic_kit_en = 'robotic-kit-en.xml'
local robotic_kit_servo_es = 'robotic-kit-servo-es.xml'
local robotic_kit_servo_en = 'robotic-kit-servo-en.xml'
...
M.execute = function(sensor, func, params, caller)
	if ((#params == 2) and (func == "setvel2mtr")) then
		local s1 = 0
		local s2 = 0		
		local v1 = tonumber(params[1])*10.23
		local v2 = tonumber(params[2])*10.23
		if(v1<0)then			
			s1 = 1
			v1 = v1 * -1
		end
	...
	if ((#params == 2) and (func == "set_vel")) then	
	
		local v1 = tonumber(params[1])
		local v2 = tonumber(params[2])
		v2 = v2 * -1		
		params={v1,v2}		
	end
		...
		if(sensor == "butia_skid") then
			...
			if ok then 
				...
				if (#sensorResult > 0) then
					for i=1, #M.active_devices do
						if (M.active_devices[i].name == sensor) then
							for j=1, #M.active_devices[i].functions do 
								if ( M.active_devices[i].functions[j] ~= nil) then
									sensor = M.active_devices[i].functions[j].butia  
									if (sensor ~= nil) then
...
M.stopActuators = function()
	if (devices['butia_skid'] ~= nil) then
		M.execute('butia_skid','set_vel', {0,0}, -1)
...
M.list_devices_functions = function(device_type)	
	...
	if (yatayLang == 'es') then
		if (devices['butia_skid'] ~= nil) then
			name_file = robotic_kit_servo_es
		else
			name_file = robotic_kit_es
		end
	else 
		if (devices['butia_skid'] ~= nil) then
			name_file = robotic_kit_servo_en
		else
			name_file = robotic_kit_en
		end
	end

    local ok, file = pcall(function() return xml.load(name_file) end)
