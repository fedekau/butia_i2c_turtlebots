...
#        self.metadata['title'] = value['name']
    ...
    def makebutton_cb(self, __):
        ...
        app_temp = mkdtemp('.activity', 'Butialo',
                           os.path.join(self.get_activity_root(), TMPDIR))
        sourcefile = os.path.join(app_temp, 'xyzzy.lua')
...
def pippy_activity_class():
    ...
    return 'pippy_app.ButialoActivity'
...
def pippy_activity_mime_types():
    ...
    return ['text/x-lua', groupthink_mimetype]
...
def main():
    ...
    for func in bundle_info.keys():
        p_a_func = 'butialo_activity_%s' % func
        if p_a_func in info:
            try_import = True
    if try_import:
    ...
    app_temp = mkdtemp('.activity', 'Butialo')  # hope TMPDIR is set correctly!
