...
modules_help['grayscale'] = _("returns the object gray level encountered him as a number between 0 and 255")
modules_help['pushbutton'] = _("Returns one when the button is press and 0 otherwise")
modules_help['ambientlight'] = _("Returns the level of ligth in the ambient as a number between 0 and 255")
...
modules_name_from_label = {} 
modules_name_from_label['led'] = 'led'
modules_name_from_label['pushbutton'] = 'boton'
modules_name_from_label['grayscale'] = 'grises'
modules_name_from_label['ambientlight'] = 'luz'
modules_name_from_label['temperature'] = 'temp'
modules_name_from_label['distance'] = 'dist'
modules_name_from_label['tilt'] = 'tilt'
modules_name_from_label['magneticinduction'] = 'magnet'
modules_name_from_label['vibration'] = 'vibra'

            
class Butia_plugin(gobject.GObject):
    ...
    def setup(self):
        ...
        palette.add_block('delayButia',  # the name of your block
                     ...
                     label=_('Delay'),  # the label for the block
					 default=[1],
...
#            
            ...
            for j in listofmodules:
                if blockstyle == 'basic-style-1arg':
                    ...
                    label=_( j + ' Butia'),  # the label for the block
                ...
                if self.butia.isPresent(modules_name_from_label[j]) == False: 
    ...
    def set_vels(self, left, right):
        ...
        if right>0:
					...
					sentRight = "1"
        self.butia.setVelocidadMotores(sentLeft,str(abs(math.trunc(left))), sentRight, str(abs(math.trunc(right))))
    ...
    def bobot_launch(self):
        ...
        """
        debug_output("initializing butia ...")
        ...
        for y in x:
            ...
            if p >= 0: # process running
                ...
                bobotAlive = False		
        if not bobotAlive:		
            #debug_output("entre en bobotinit")
            cmd = './plugins/butia_support/bobot_kill.sh'
            os.system(cmd)
            cmd = './plugins/butia_support/bobot_init.sh'
            os.system(cmd)
